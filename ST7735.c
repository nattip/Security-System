...
void Clock_Init48MHz(void) // sets the clock module to use the
{
    MAP_GPIO_setAsPeripheralModuleFunctionOutputPin(
            GPIO_PORT_PJ, GPIO_PIN3 | GPIO_PIN2, GPIO_PRIMARY_MODULE_FUNCTION); //sets pins for crystal clock usage
    CS_setExternalClockSourceFrequency(32000, 48000000); //sets low and high frequency oscillator speeds
    MAP_PCM_setCoreVoltageLevel(PCM_VCORE1);          //allows support for 48MHz
    MAP_FlashCtl_setWaitState(FLASH_BANK0, 2);
    MAP_FlashCtl_setWaitState(FLASH_BANK1, 2);
    CS_startHFXT(false); // false means that there are no timeouts set,will return when stable

    MAP_CS_initClockSignal(CS_MCLK, CS_HFXTCLK_SELECT, CS_CLOCK_DIVIDER_1); //MCLK run on high req oscillator at 48MHz
    MAP_CS_initClockSignal(CS_SMCLK, CS_HFXTCLK_SELECT, CS_CLOCK_DIVIDER_4); //SMCLK run on high freq oscillator at 48MHz divided by 4
    ...
    MAP_CS_initClockSignal(CS_BCLK, CS_REFOCLK_SELECT, CS_CLOCK_DIVIDER_1); //Set A clock to REFO clock source divider 1
}
